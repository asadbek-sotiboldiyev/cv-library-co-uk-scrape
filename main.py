...
from utils import accept_all_cookies
from data_saving_types import save_all_in_one


print("Starting app...")

# original_out = sys.stdout
log_file_name = datetime.strftime(datetime.now(), '%Y_%m_%d_%H_%M_%S')
# sys.stdout = open(log_file_name + "_log.txt", 'w', encoding="utf-8")
# sys.stderr = open(log_file_name + "_log.txt", 'w', encoding="utf-8")
...
tools_list = list({".NET", "Adobe XD", "Airflow", "Alamofire", "Alexa Skills Kit", "Amplify", "Ansible", "Apache", "Apache Kafka",
     ...
     "Bootstrap", "C++", "Caffe", "Caossandra", "Chef", "Cisco ASA", "Cisco Packet Tracer", "CircleCI", "ClickHouse",
...
categoryname = 'categories=it' # it
orderby = '&order=date'

# last [posted_day_ago] days
posted_day_ago = 2 # so'nggi necha kunni olsih kerakligi
postedwhen = '&posted=' + str(posted_day_ago)

link = f'https://www.cv-library.co.uk/data-analyst-jobs-in-uk?{categoryname}{orderby}{postedwhen}&us=1'
...
searchkeywords = ['Data engineer', 'Android developer', 'Data scientist',
       'AI engineer', 'Game developer', 'IOS developer',
       'DevOps engineer', 'Cybersecurity Analyst', 'IT project manager',
       'Network engineer', 'Cloud Architect', 'Full stack developer',
       'Data analyst', 'Frontend developer', 'Backend developer', 'QA engineer']

# hamma data lar olinib keyin databasega yoziladi
save_all_in_one(driver, searchkeywords, tools_list, log_file_name)
